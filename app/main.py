...
from app.observability import invoke_agent_observed
from app.provenance import build_provenance_ledger, append_provenance_ledger
from app.schema import dynamic_schema, LOADED_FISCAL_YEARS, DATASET_PROVENANCE


api = FastAPI(
    title="Building Data AI Agent",
    description=(
...
def schema():
    return {
        "tables": list(dynamic_schema.keys()),
        "columns": {
            table_name: [
                {
            ...
            for table_name, columns in dynamic_schema.items()
        },
        "fiscal_years": LOADED_FISCAL_YEARS,
        "dataset": DATASET_PROVENANCE,
    }


@api.post("/query", response_model=AgentQueryResponse)
def query_agent(request: AgentQueryRequest):
    question = request.question.strip()

    try:
        result = invoke_agent_observed(question)
...
@api.post("/query/provenance")
def query_with_provenance(request: AgentQueryRequest):
    question = request.question.strip()

    try:
        ledger = build_provenance_ledger(question)
        append_provenance_ledger(ledger)
        return ledger
    except Exception as error:
        raise HTTPException(
            status_code=500,
            detail="The agent could not complete the provenance request.",
        ) from error
